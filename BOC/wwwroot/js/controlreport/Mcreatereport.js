$(function () {
    ...
    $('.custom-file-upload').bind('touchstart click', function () {
     ...
     $('#fileToUpload').on('change', function () {
         ...
         if (size > ((1024*1024)*10)) {
             ...
             $.ajax({
                 type: "POST",
                 url: "/CR/ControlReportMobile/UploadFileAttach",
                 data: formData,
    ...
    $(this).on("touchstart click", ".removeFile", function () {
        var formData = new FormData();
        ...
        $.ajax({
            type: "POST",
            url: "/CR/ControlReportMobile/RemoveAttachFile",
            data: formData,
            //use contentType, processData for sure.
            contentType: false,
            processData: false,
            ...
            success: function (data) {
                ...
                if (data.mess = "OK") {
                    ...
                    for (let i = 0; i < json.length; i++) {
                        var filepath = "/data/ControlReport/" + '/' + json[i].sysFileName;
                        if (json[i].Status == "OK") {
                            var obj = "<li>" + json[i].FileName + "&nbsp&nbsp-&nbsp&nbsp" +"<br>" + "<a  href=" + filepath + " ' target='_blank'" + " ' style='display: inline-block; width: 20px; height: 20px;color:blue;'" + "\" >View</a>" + "&nbsp&nbsp&nbsp&nbsp" + "<a class=\"removeFile\" href=\"#\"  style=\"display: inline - block; width: 20px; height: 20px; color: red;\" data-fileid=\"" + json[i].sysFileName + "\" >Remove</a>" + "</li> ";
                            chuoi = chuoi + obj;
                        }
    ...
    $(document).on('touchstart click', '#sendreport', function () {
        var formData = new FormData();
        ...
        $.ajax({
            type: "POST",
            /* url: '@Url.Action("CreateReport", "ControlReportMobile")',*/
            url: "/CR/ControlReportMobile/CreateReport",
            data: formData,
            processData: false,
            contentType: false,
            cache: false,
            success: function (data) {
                ...
                if (data.mess == "OK") {
                    HideProgressAnimation();
                    window.location.href = "/CR/ControlReportMobile/ReportList";
                }
