...
namespace BOC.Controllers
{
    ...
    [ApiController]
    public class Report1_ApiController : ControllerBase
    {
        [HttpGet]
        public object Get(DataSourceLoadOptions loadOptions,  string fromdate1, string todate1, string fromdate2, string todate2)
        {
            ...
            string uri = flightloadfactor.Get("FlightLoadFactor");
            ...
            foreach (var item in data)
            {
                ...
                lstrpt1.Station = _Report1Item.ToString();

                _Report1Item = item.SelectToken("AirportName");
                lstrpt1.AirportName = _Report1Item.ToString();

                _Report1Item = item.SelectToken("FltKy1");
                lstrpt1.FltKy1 = Int32.Parse(_Report1Item.ToString());

                _Report1Item = item.SelectToken("FltKy2");
                lstrpt1.FltKy2 = Int32.Parse(_Report1Item.ToString());

                _Report1Item = item.SelectToken("PaxC_Ky1");
                lstrpt1.PaxC_Ky1 = Int32.Parse(_Report1Item.ToString());

                _Report1Item = item.SelectToken("PaxC_Ky2");
                lstrpt1.PaxC_Ky2 = Int32.Parse(_Report1Item.ToString());

                _Report1Item = item.SelectToken("PaxY_Ky1");
                lstrpt1.PaxY_Ky1 = Int32.Parse(_Report1Item.ToString());

                _Report1Item = item.SelectToken("PaxY_Ky2");
                lstrpt1.PaxY_Ky2 = Int32.Parse(_Report1Item.ToString());

                _Report1Item = item.SelectToken("Config_Ky1");
                lstrpt1.Config_Ky1 = Int32.Parse(_Report1Item.ToString());

                _Report1Item = item.SelectToken("Config_Ky2");
                lstrpt1.Config_Ky2 = Int32.Parse(_Report1Item.ToString());

                _Report1Item = item.SelectToken("Tong_Ky1");
                lstrpt1.Tong_Ky1 = Int32.Parse(_Report1Item.ToString());

                _Report1Item = item.SelectToken("Tong_Ky2");
                lstrpt1.Tong_Ky2 = Int32.Parse(_Report1Item.ToString());

                if (lstrpt1.Config_Ky1 == 0)
                {
                    
                    lstrpt1.Load_Factor1 =  "0%";
                }
                else
                {
                     int loadfactor1 = (int)Math.Round(((double)(100 * lstrpt1.Tong_Ky1) / lstrpt1.Config_Ky1),2);
                     lstrpt1.Load_Factor1 = loadfactor1.ToString() + '%';
                }
                if (lstrpt1.Config_Ky2 == 0)
                {

                    lstrpt1.Load_Factor2 = "0%";
                }
                else
                {
                    int loadfactor2 = (int)Math.Round(((double)(100 * lstrpt1.Tong_Ky2) / lstrpt1.Config_Ky2),2);
                    lstrpt1.Load_Factor2 = loadfactor2.ToString() + '%';
                }
            ...
            //List<RPT_GO_1> lstrpt1 = ser.Deserialize<List<RPT_GO_1>>(oData["Data"].ToString());//str is JSON string.
            //for (int i = 0; i < lstrpt1.Count; i++)
            //{
            //    lstrpt1[i].ID = i + 1;
            //}
